...
export default function VerificationPage() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCountdown((prev) => {
        if (prev <= 1) {
          clearInterval(timer)
          // Redirect to /chat if authenticated, otherwise homepage
          router.push(isAuthenticated ? '/chat' : '/')
          return 0
        }
        return prev - 1
      })
  ...
  const redirectUrl = isAuthenticated ? '/chat' : '/'
  const redirectText = isAuthenticated ? 'Continue to Chat' : 'Continue to Homepage'
