...
export async function GET(req: NextRequest) {
  try {
    ...
    const referenceId = searchParams.get('reference-id') // This could be user_id OR session_token
    ...
    // Look up session by inquiry ID first
    let session = null
    let sessionError = null

    const { data: sessionByInquiry, error: inquiryLookupError } = await adminSupabase
      .from('kyc_sessions')
      .select('*')
    ...
    if (sessionByInquiry) {
      session = sessionByInquiry
    } else if (referenceId && !sessionByInquiry) {
      // Try to find session by session_token (reference-id)
      // This handles the new desktop QR flow where session_token is used as reference-id
      const { data: sessionByToken, error: tokenLookupError } = await adminSupabase
        .from('kyc_sessions')
        .select('*')
        .eq('session_token', referenceId)
        .maybeSingle()

      if (sessionByToken) {
        session = sessionByToken
        console.log('[KYC Callback] Session found by session_token (desktop QR flow):', {
          sessionId: sessionByToken.id,
          userId: sessionByToken.user_id,
          inquiryId,
          sessionToken: referenceId.substring(0, 8) + '...',
          timestamp: new Date().toISOString(),
        })
      } else {
        sessionError = tokenLookupError
      }
    } else {
      sessionError = inquiryLookupError
    }
    ...
    console.log('[KYC Callback] Session found successfully:', {
      sessionId: session.id,
      userId: session.user_id,
      inquiryId: session.inquiry_id,
      initiatedFrom: session.initiated_from,
    ...
    try {
      ...
      if (verificationStatus === 'approved' && accountId) {
        ...
        if (accountError) {
          ...
          console.log('[KYC Callback] Persona account created successfully:', {
            userId: session.user_id,
            accountId,
            timestamp: new Date().toISOString(),
          })
        }
      } else if (verificationStatus === 'approved' && !accountId) {
        // No account ID yet from Persona - this can happen if account hasn't been assigned yet
        // The persona_accounts record will be created later via webhook or manual review
        console.warn('[KYC Callback] Approved verification without account ID:', {
          userId: session.user_id,
          inquiryId,
          verificationStatus: 'approved',
          sessionStatus,
          note: 'persona_accounts record not created - account ID not yet assigned by Persona',
          timestamp: new Date().toISOString(),
        })
      }
      ...
      if (sessionStatus === 'completed' && verificationStatus === 'approved') {
        ...
        const redirectPath = session.initiated_from === 'desktop_qr'
          ? '/kyc/complete-on-web'
          : '/kyc/success'

        console.log('[KYC Callback] Routing decision - verification approved:', {
          sessionId: session.id,
          userId: session.user_id,
          initiatedFrom: session.initiated_from,
          deviceType: session.device_type,
        ...
        console.log('[KYC Callback] Routing decision - verification failed:', {
          sessionId: session.id,
          userId: session.user_id,
          sessionStatus,
          verificationStatus,
          redirectPath: '/kyc/fail',
          timestamp: new Date().toISOString(),
        })
        return NextResponse.redirect(`${appUrl}/kyc/fail`)
      } else if (verificationStatus === 'needs_review') {
        console.log('[KYC Callback] Routing decision - verification needs review:', {
          sessionId: session.id,
          userId: session.user_id,
          sessionStatus,
          verificationStatus,
          redirectPath: '/kyc/pending',
          timestamp: new Date().toISOString(),
        })
        return NextResponse.redirect(`${appUrl}/kyc/pending`)
      } else {
        // Still in progress or pending
        console.log('[KYC Callback] Routing decision - verification still pending:', {
          sessionId: session.id,
          userId: session.user_id,
          sessionStatus,
          verificationStatus,
