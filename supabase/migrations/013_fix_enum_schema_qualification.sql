-- Fix enum schema qualification issues that cause "type action_type does not exist" errors
-- This migration adds proper schema qualification and search paths to all functions
-- Issue: Functions called from auth schema triggers can't find public schema types

-- STEP 1: Drop existing functions to recreate with proper qualification
...
DROP FUNCTION IF EXISTS public.award_signup_bonus(UUID) CASCADE;
...
-- STEP 5: Create helper function for awarding signup bonus with proper qualification
CREATE OR REPLACE FUNCTION public.award_signup_bonus(p_user_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT EXISTS(
    ...
    AND ui.action_type = 'signup'::public.action_type
  ) INTO v_has_bonus;
  ...
  -- Award the bonus
  v_transaction_id := public.award_equity_points(
    p_user_id,
...
EXCEPTION WHEN OTHERS THEN
  RAISE WARNING 'Failed to award signup bonus to user %: %', p_user_id, SQLERRM;
  RETURN FALSE;
END;
$$;

-- STEP 6: Recreate triggers to ensure they use the updated functions
...
-- STEP 7: Fix any other functions that might use enums
-- Update complete_referral if it exists
DO $$
BEGIN
  IF EXISTS (
    ...
    -- Drop and recreate with proper qualification
...
-- STEP 8: Create a test to verify the fix works
DO $$
DECLARE
  test_id UUID := gen_random_uuid();
  test_email TEXT := 'enum_test_' || substr(md5(random()::text), 1, 8) || '@example.com';
  test_result BOOLEAN;
BEGIN
  ...
  INSERT INTO auth.users (
    ...
    jsonb_build_object('full_name', 'Enum Test User'),
    NOW(),
    NOW(),
  ...
  ELSE
    RAISE WARNING '❌ ENUM FIX FAILED - Check function definitions';
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  RAISE WARNING '❌ Test failed with error: % (State: %)', SQLERRM, SQLSTATE;
  -- Clean up on error
...
GRANT EXECUTE ON FUNCTION public.award_signup_bonus TO service_role;

-- Final status
SELECT 
  '✅ Enum schema qualification fix applied' as status,
  'Functions now properly handle cross-schema enum references' as description,
  'Email verification should now work without enum type errors' as result;